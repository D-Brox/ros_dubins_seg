            ...
            for i,j in combinations(range(self.__n_robots),2):
                if (self.__x[i] - self.__x[j])**2 + (self.__y[i] - self.__y[j])**2 < self.__params['c']**2:
                    try:
                        ...
                        resp_i_recv_j = receive_memory_i(memory_j.curve_index, memory_j.group, memory_j.time_curve, memory_j.time, memory_j.pose2D_x, memory_j.pose2D_y, memory_j.pose2D_theta, memory_j.mov_will, memory_j.number,memory_j.state)
                        resp_j_recv_i = receive_memory_j(memory_i.curve_index, memory_i.group, memory_i.time_curve, memory_i.time, memory_i.pose2D_x, memory_i.pose2D_y, memory_i.pose2D_theta, memory_i.mov_will, memory_i.number,memory_i.state)
                    except:
                        pass
